...
앱 실행 시 GitHub Releases를 확인해서, 새 버전이 있으면 배너로 안내하고 한 번 탭으로 다운로드할 수 있습니다 — 최초 설치 이후로는 adb/케이블이 필요 없습니다. 다만 Android 정책상 설치 시점에는 매번 시스템 확인 다이얼로그를 한 번 거쳐야 합니다 (Play 스토어를 거치지 않는 앱은 이 부분을 자동화할 수 없습니다) — 대신 수동으로 APK를 복사하던 과정은 없어집니다.

새 버전을 배포하려면(관리자용):

1. `app/build.gradle.kts`에서 `versionCode`/`versionName`을 올립니다.
2. 커밋 후 태그를 붙여 푸시: `git tag v<versionCode> && git push origin v<versionCode>` (예: `v3`).
3. `.github/workflows/release.yml` 워크플로우가 자동으로 APK를 빌드해서 GitHub Release로 발행합니다 — 앱의 업데이트 확인 기능은 `v<versionCode>` 태그 형식과 그 릴리스에 첨부된 `.apk` 에셋을 그대로 사용합니다.

이 기능은 저장소가 Public일 때만 동작합니다 — 업데이트 확인 로직이 별도 인증 토큰 없이 GitHub API를 호출하도록 설계되어 있기 때문입니다 (APK에 토큰을 심으면 추출 가능해서 의도적으로 그렇게 만들지 않았습니다).
